...
/**
 * Wrapper class to process provider calls and fill in necessary information
 * before passing on to native EclipseLink provider class.
 */
...
public class ProviderWrapper implements PersistenceProvider {
	...
	public EntityManagerFactory createEntityManagerFactory(String emName,
		...
		debug("ProviderWrapper createEMF invoked for p-unit: ", emName,
				" props: ", properties);

		PUnitInfo unitInfo = mgr.getPUnitsByName().get(emName);
		if (unitInfo == null)
			fatalError(
					"createEntityManagerFactory() called on provider, but punit has not been registered: ",
					null);
		...
		// Now the props passed into this createEMF call (may overwrite config
		// admin props)
		if (properties != null) {
			props.putAll(properties);
		}

		// Create a composite loader that loads from the punit bundle and the
		// provider bundle
		CompositeClassLoader compositeLoader = CompositeClassLoader
				.createCompositeLoader(mgr.getBundleContext(),
						unitInfo.getBundle());
		...
		if (props.containsKey(PersistenceUnitProperties.CLASSLOADER)) {
			ClassLoader userLoader = (ClassLoader) props
					.get(PersistenceUnitProperties.CLASSLOADER);
		...
		DataSource ds = mgr.getDataSourceUtil().acquireDataSource(unitInfo,
				properties);
		if (ds != null)
		...
		props.put(PersistenceUnitProperties.ECLIPSELINK_PERSISTENCE_XML,
				fullDescriptorPath(unitInfo));

		// Put in a private property that we can use later on in the call stack
		// to get the punit info
		props.put(PUNIT_INFO_PROPERTY, unitInfo);

		// TODO Pirchner
		if (unitInfo.getValidationFactory() != null) {
			props.put(PersistenceUnitProperties.VALIDATOR_FACTORY,
					unitInfo.getValidationFactory());
		}

		// Now make the call
		EntityManagerFactory emf = nativeProvider.createEntityManagerFactory(
				emName, props);
		return emf;
	}
	...
	public boolean generateSchema(String unitName, Map properties) {
		debug("ProviderWrapper generateSchema invoked for p-unit: ", unitName,
				" props: ", properties);
	...
	protected void initializeForWeaving(ClassLoader cl,
