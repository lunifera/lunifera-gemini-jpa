...
public class ServicesUtil {
	...
	/**
	 * Register the provider as a persistence provider service. The service
	 * registration will be stored locally.
	 */
	public void registerProviderService() {
		...
		// Service strings
		String[] serviceNames = { javax.persistence.spi.PersistenceProvider.class
				.getName() };
		// Get a provider JPA SPI instance
		javax.persistence.spi.PersistenceProvider persistenceProvider = mgr
				.getProvider();
		...
		providerService = mgr.getBundleContext().registerService(serviceNames,
				persistenceProvider, props);
		debug("ServicesUtil successfully registered provider service for ",
				providerClassName);
	}
	...
	public void unregisterProviderService() {
		...
		debug("ServicesUtil successfully un-registered provider service for ",
				providerClassName);
	}

	/**
	 * Register a weaving hook whiteboard service. Called by the
	 * GeminiOSGiInitializer.registerTransformer method. The service
	 * registration will be stored locally.
	 */
	public void registerWeavingHookService(WeavingHookTransformer weaver,
			PUnitInfo pUnitInfo) {

		GeminiUtil.debugWeaving(
				"ServicesUtil.registerWeavingHookService for punit ",
				pUnitInfo.getUnitName());
		Hashtable<String, Object> props = new Hashtable<String, Object>();
		props.put("osgi.unit.name", pUnitInfo.getUnitName());

		ServiceRegistration svcReg = mgr.getBundleContext().registerService(
				WeavingHook.class.getName(), weaver, props);
		pUnitInfo.setWeavingHookService(svcReg);
		debug("ServicesUtil successfully registered weaving hook for ",
				pUnitInfo.getUnitName());
	}
	...
	public void unregisterWeavingHookService(PUnitInfo pUnitInfo) {
		...
		debug("ServicesUtil.unregisterWeavingHookService for ",
				pUnitInfo.getUnitName());
		ServiceRegistration svcReg = pUnitInfo.getWeavingHookService();
		if (svcReg != null) {
			svcReg.unregister();
			pUnitInfo.setWeavingHookService(null);
			debug("ServicesUtil successfully un-registered weaving hook for ",
					pUnitInfo.getUnitName());
		}
	}
	...
	public void registerEMFServices(PUnitInfo pUnitInfo) {
		...
		// Try to register the EMF service (it will only occur if data source is
		// available)
	 ...
	 * 
	 * @param pUnitInfo
	 *            The metadata info for the persistence unit
	 * @param anchorClasses
	 *            Set of classes to be proxied and the service is to be
	 *            registered against
	 * @param serviceProps
	 *            Service properties to use when registering the service
	 */
	public void tryToRegisterEMFService(PUnitInfo pUnitInfo,
			Map<String, Class<?>> anchorClasses,
			Dictionary<String, String> serviceProps) {

		debug("ServicesUtil.tryToRegisterEMF service for ",
				pUnitInfo.getUnitName());
		// Array of classes being proxied by EMF proxy
		Collection<Class<?>> proxiedClasses = new ArrayList<Class<?>>();

		// Load the EMF class
		Class<?> emfClass = GeminiUtil.loadClassFromBundle(
				"javax.persistence.EntityManagerFactory", mgr.getBundle());

		// Add EMF class and anchor classes to the proxied class collection for
		// EMF proxy
		...
		if (nonJtaDataSource != null) {
			...
			DataSource ds = mgr.getJndiUtil()
					.lookupDataSource(nonJtaDataSource);
			if (ds == null) {
				warning("Non-JTA data source " + nonJtaDataSource
						+ " was not found. EMF service not registered.");
				return;
			} else {
			...
			if (!mgr.getDataSourceUtil().trackDataSourceFactory(pUnitInfo)) {
				...
				debug("DataSourceFactory service for " + driverClassName
						+ " not found.");
				// Driver may be local (packaged in with the p-unit) -- try
				// loading it
				try {
					pUnitInfo.getBundle().loadClass(driverClassName);
					debug("JDBC driver " + driverClassName + " found locally.");
					// We found the driver in the punit. Stop tracking DBAccess
					// service and revert to direct access
					mgr.getDataSourceUtil().stopTrackingDataSourceFactory(
							pUnitInfo);
				} catch (ClassNotFoundException cnfEx) {
					// Driver not local, bail and wait for the tracker to detect
					// DBAccess service
					debug("JDBC driver " + driverClassName
							+ " was not found locally.");
					warning("DataSourceFactory service for " + driverClassName
							+ " was not found. EMF service not registered.");
					return;
				}
			}
		...
		// TODO Pirchner
		String valFactory = pUnitInfo.getValidationJndiName();
		if (valFactory != null) {
			try {
				debug("JndiUtil.lookupValidatorFactory - looking up ",
						valFactory);
				Context ctx = new InitialContext();
				Object result = ctx.lookup(valFactory);
				if (result instanceof ValidatorFactory) {
					pUnitInfo.setValidationFactory((ValidatorFactory) result);
				}
			} catch (NameNotFoundException nnfEx) {
				debug("JndiUtil.lookupValidatorFactory - Name not found for ",
						valFactory);
			} catch (Throwable t) {
				fatalError("Lookup for " + valFactory + " failed: ", t);
			}
		}
		...
		// Register the EMF service (using p-unit context) and set registration
		// in PUnitInfo
		ServiceRegistration emfService = null;
		try {
			emfService = pUnitInfo
					.getBundle()
					.getBundleContext()
					.registerService(classNameArray, emfServiceProxy,
							serviceProps);
			debug("ServicesUtil EMF service: ", emfService);
		} catch (Exception e) {
			fatalError("ServicesUtil could not register EMF service for "
					+ pUnitInfo.getUnitName(), e);
		}
	...
	public void registerEMFBuilderService(PUnitInfo pUnitInfo,
		...
		debug("ServicesUtil.registerEMFBuilder service for ",
				pUnitInfo.getUnitName());
		// Array of classes being proxied by EMFBuilder proxy
		Collection<Class<?>> proxiedClasses = new ArrayList<Class<?>>();

		// Load the EMFB class
		Class<?> emfBuilderClass = GeminiUtil.loadClassFromBundle(
		...
		// Get a loader to load the proxy classes
		ClassLoader loader = proxyLoader(pUnitInfo, anchorClasses,
				emfBuilderClass);

		// Create proxy impl object for EMF service
		Object emfBuilderServiceProxy = createEMFBuilderProxy(pUnitInfo,
				loader, classArray);
		...
		try {
			...
			emfBuilderService = mgr.getBundleContext().registerService(
					classNameArray, emfBuilderServiceProxy, props);
			debug("ServicesUtil EMFBuilder service: ", emfBuilderService);
		} catch (Exception e) {
			fatalError(
					"ServicesUtil could not register EMFBuilder service for "
							+ pUnitInfo.getUnitName(), e);
		}
	...
	/**
	 * Unregister the EMF service if there was an EMF service registered. Clean
	 * up any resources the service may have allocated.
	 * 
	 * @param pUnitInfo
	 */
	public void unregisterEMFService(PUnitInfo pUnitInfo) {
		...
		if (emfService != null) {
			debug("ServicesUtil un-registering EMF service for ",
					pUnitInfo.getUnitName());
			try {
			...
			debug("ServicesUtil un-registered EMF service for ",
					pUnitInfo.getUnitName());
	...
	/**
	 * Unregister the EMFBuilder service. Clean up any resources the service may
	 * have allocated.
	 * 
	 * @param pUnitInfo
	 */
	public void unregisterEMFBuilderService(PUnitInfo pUnitInfo) {
		...
		debug("ServicesUtil.unregisterEMFBuilderService for ",
				pUnitInfo.getUnitName());

		// Unregister the service
		ServiceRegistration emfBuilderService = pUnitInfo
				.getEmfBuilderService();
		if (emfBuilderService != null) {
			debug("ServicesUtil un-registering EMFBuilder service for ",
					pUnitInfo.getUnitName());
			try {
			...
			debug("ServicesUtil un-registered EMFBuilder service for ",
					pUnitInfo.getUnitName());
	...
	/**
	 * Get or create a loader to load classes from the punit. A sequence of
	 * conditions provides a pattern for obtaining it.
	 */
	ClassLoader extractPUnitLoader(PUnitInfo pUnitInfo,
		...
		if (!anchorClasses.isEmpty()) {
			pUnitLoader = anchorClasses.values().iterator().next()
					.getClassLoader();

			// 2. Otherwise, if there are managed JPA classes listed, use one to
			// get the loader
		} else if (!pUnitInfo.getClasses().isEmpty()) {
			try {
				pUnitLoader = pUnitInfo
						.getBundle()
						.loadClass(
								(String) (pUnitInfo.getClasses().toArray()[0]))
						.getClassLoader();
			} catch (ClassNotFoundException cnfEx) {
	...
	ClassLoader proxyLoader(PUnitInfo pUnitInfo,
			Map<String, Class<?>> anchorClasses, Class<?> jpaClass) {

		ClassLoader cl = null;

		// If there are no managed JPA classes listed, return loader used to
		// load the class passed in
	 ...
	 */
	Object createEMFProxy(PUnitInfo pUnitInfo, ClassLoader loader,
			Class<?>[] clsArray) {

		EMFServiceProxyHandler emfProxyHandler = new EMFServiceProxyHandler(
				pUnitInfo);
		Object result = null;
		try {
			...
			fatalError(
					"ServicesUtil - Failed to create proxy for EMF service: ",
					e);
		}
	...
	/**
	 * Create and return a proxy for the EMFBuilder (and specified list of
	 * classes which must include the EMFBuilder class).
	 */
	Object createEMFBuilderProxy(PUnitInfo pUnitInfo, ClassLoader loader,
			Class<?>[] clsArray) {

		// Assume that EMF proxy handler has been created and is stored in
		// pUnitInfo
		EMFBuilderServiceProxyHandler emfBuilderProxyHandler = new EMFBuilderServiceProxyHandler(
				pUnitInfo, pUnitInfo.getEmfHandler());
		Object result = null;
		try {
			result = Proxy.newProxyInstance(loader, clsArray,
					emfBuilderProxyHandler);
			debug("ServicesUtil created EMFBuilder proxy ");
		} catch (Exception e) {
			fatalError(
					"ServicesUtil - Failed to create proxy for EMFBuilder service: ",
					e);
		}
	...
	public Dictionary<String, String> buildServiceProperties(PUnitInfo pUnitInfo) {
		...
		String bundleId = pUnitBundle.getSymbolicName() + "_"
				+ bundleVersion(pUnitBundle);
