...
/**
 * Parser handler for parsing the persistence descriptors. This class does not
 * parse the descriptors in their entirety, but saves only a portion of the
 * content in order for the OSGi integration to know whether a given persistence
 * unit can be assigned to a particular provider.
 */
public class PersistenceDescriptorHandler extends DefaultHandler {
	...
	public void startElement(String uri, String localName, String qName,
		...
		if (currentElement.equals("persistence-unit")) {
			...
			if (nullOrEmpty(propName))
				GeminiUtil.fatalError(
						"Invalid 'name' for persistence descriptor <property>",
						null);
			...
			if (propName.equals(GeminiUtil.JPA_JDBC_DRIVER_PROPERTY))
				...
				currentPUnit.setDriverPassword(propValue);
					...
					.equals(GeminiUtil.OSGI_JDBC_DRIVER_VERSION_PROPERTY))
				currentPUnit.setDriverVersion(propValue);
			// TODO pirchner
			else if (propName.equals(GeminiUtil.VALIDATOR_FACTORY_PROPERTY))
				currentPUnit.setValidationJndiName(propValue);
		}
	}
	...
	public void characters(char[] ch, int start, int length)
		...
		if (currentElement.equals("provider")) {
			...
			currentPUnit
					.setNonJtaDataSource(contentFromChars(ch, start, length));
